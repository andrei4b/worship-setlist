...
const MAIL_ICON = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M3 7l9 6 9-6"/></svg>`;
const SIGNOUT_ICON = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><path d="M16 17l5-5-5-5"/><path d="M21 12H9"/></svg>`;
...
function accountMenuItems() {
  const items = [];
  if (Auth.isAdmin()) {
    items.push({ icon: MAIL_ICON, label: 'Invite people', onClick: showInviteCode });
    items.push({ icon: PEOPLE_ICON, label: 'Manage members', onClick: showMembers });
  }
  items.push({ icon: SIGNOUT_ICON, label: 'Sign out', onClick: () => Auth.signOut() });
  return items;
}
